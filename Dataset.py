    ...
    def get_prob(self, i, k):
        ...
        if len(self.net.nodes[k].parents) == 1:  # nodo con un padre p(T|A)
            if self.dataset[i][self.net.nodes[k].parents[0]] == 0:
                prob = self.net.nodes[k].cpt[1][0]  # A =  f
            else:
                prob = self.net.nodes[k].cpt[0][1]  # A =  t

        if len(self.net.nodes[k].parents) == 2:  # nodo con 2 padri p(E|T,L)
            if self.dataset[i][self.net.nodes[k].parents[0]] == 0:  # T = f
                if self.dataset[i][self.net.nodes[k].parents[1]] == 0:  # L = f
                    prob = self.net.nodes[k].cpt[3][0]
                else:
                    prob = self.net.nodes[k].cpt[1][0]  # T = f & L = t

            if self.dataset[i][self.net.nodes[k].parents[1]] == 1:  # L = T
                if self.dataset[i][self.net.nodes[k].parents[0]] == 1:  # L = T & T = t
